...
\usepackage{polski} \usepackage[cp1250]{inputenc} \usepackage[polish]{babel} % Te pakiety umo¿liwiaj¹ sk³adanie pracy w jêzyku polskim
...
{\large \textbf{Uniwersytet Jagielloñski w Krakowie}
...
\chapter*{Wstêp}
	...
	Wybór tematu poni¿szej pracy jest uzasadniony zainteresowaniami autora w dziedzinie robotyki i elektroniki, jak równie¿
...
Podstaw¹ w poprawnym dzia³aniu takich ramion jest odpowiednie zaprojektowanie, ale nawet najbardziej dopracowana maszyna, mo¿e byæ bezu¿yteczna,
...
\indent
	Najbardziej popularne i ogólnie dostêpne czêœci na rynku elektronicznym s¹ zazwyczaj
bardzo prymitywne w budowie, co oczywiœcie ma swoje zalety i wady. Do tych drugich nale¿y awaryjnoœæ, ale nie spowodowana b³êdami projektowymi czy produkcyjnymi, a nieumiejêtnoœci¹ i brakiem rozwagi u¿ytkowników.
...
\indent
Kompletne Ÿród³a programów, napisane na potrzeby poni¿szej pracy, znajduj¹ siê na noœniku za³¹czonym do pracy.
\\ \\
\indent
...
Do budowy stawów, o których mowa w rozdziale 1.1 wykorzystano serwa modelarskie SG90 - jedne 
z najbardziej popularnych i ³atwo dostêpnych serw na rynku, które charakteryzuj¹ siê równie¿
niskimi cenami.  Poœrednikiem pomiêdzy serwami a komputerem, który zapewni odpowiedni¹ kontrolê serw 
...
rzeczywistym przesy³aæ instrukcje steruj¹ce z komputera i informacje o przeci¹¿eniach do komputera. \\ 
\indent
...
w serwach analogowych, jakim jest SG90, to ok. 50 Hz. W serwach cyfrowych jest to 300 Hz i wp³ywa to na ich wiêksz¹ precyzjê.
...
ró¿nicê w pozycjach, a¿ do uzyskania po¿¹danego efektu.
\begin{figure}[H]
	\centering
...
z których jest wykonane serwo. W opisywanym projekcie autor wykorzysta³ serwa SG90 firmy Tower Pro. 
...
\section{Pomiar ró¿nicowy, przewaga nad innymi rozwi¹zaniami }

W podrozdziale 2.1 zosta³a opisana zasada ustalania i zmian pozycji serwa, która polega na porównywaniu napiêæ. 
...
pozycji korektor bêdzie usilnie chcia³ odpowiednio ustawiæ silnik. Takie dzia³anie mo¿e prowadziæ do spalenia silnika lub
...
a dopiero powsta³e tam napiêcie na wyjœciu trafia bezpoœrednio do przetwornika ADC (Analog-\(>\)Digital Converter).
\begin{figure}[H]
	\centering
...
czyli ró¿nica napiêæ w nich panuj¹cych bêdzie identyczna niezale¿nie od zaburzeñ zewnêtrznych. 
\begin{figure}[H]
	\centering
...
wbudowane w mikrokontroler maj¹ ograniczon¹ szybkoœæ, zw³aszcza wtedy, kiedy ten sam mikrokontroler musi zapewniæ równie¿ transmisjê
...
Jak opisano w rozdziale 2.2 pomiar ró¿nicowy wykonywany jest na elemencie elektronicznym - w tym przypadku jest to 
rezystor bocznikuj¹cy, czyli taki o bardzo ma³ej rezystancji i du¿ej precyzji przeznaczony do pomiaru du¿ych pr¹dów oraz nie 
...
a mas¹, aby zapewniæ odpornoœæ na szumy, iskrzenia silnika, poniewa¿ s¹ one elementami indukcyjnymi.
...
operacje na rejestrach steruj¹cych ATmegi. Jest to projekt stale rozwijany, posiadaj¹cy wielu zwolenników, mnóstwo ksi¹¿ek do nauki
...
ma dok³adnoœæ 4.88mV, a w przypadku pomiaru ró¿nicowego jest to 9.17mV, poniewa¿ zakres wartoœci z przetwornika to 
[-512, +511]. 
Opór opisanego w podrozdziale 2.3 rezystora bocznikuj¹cego jest bardzo ma³y, a to oznacza, ¿e spadek napiêcia mierzony na nim 
...
ró¿nica jest wzmacniana i dopiero ta wartoœæ trafia do 10-bitowego przetwornika. Dok³adnoœæ pomiaru, w przypadku 
...
Wy¿ej opisana operacja powoduje diametralne zwiêkszenie rozbie¿noœci miêdzy wartoœciami pr¹dów w ró¿nych stanach
...
gotowy do odczytu. Zamieszczony ni¿ej fragment kodu zawiera funkcje do inicjalizacji i wykonywania ró¿nicowych pomiarów napiêcia:
\begin{lstlisting}
...
mikrokontrolera - dlatego dla zachowania jednolitoœci i prostego zrozumienia kodu naturalnym by³oby wybranie w³aœnie tego jêzyka dla napisania aplikacji komputerowej.
...
o aktualnych przeci¹¿eniach serw oraz poddaje je analizie. Otrzymane sygna³y s¹ czêsto skrajne - dla braku przeci¹¿eñ pojawiaj¹ce siê wartoœci poboru pr¹du s¹ mniejsze
...
Do zmagazynowania informacji nadchodz¹cych z mikrokontrolera wykorzystano zmodyfikowan¹ kolejkê FIFO - domyœlna kolejka zaimplementowana w C\# jest oparta o listê
wskaŸnikow¹, czyli teoretycznie o nieskoñczonej pojemnoœci i braku dostêpu do konkretnej liczby elementów wstecz. Spersonalizowana kolejka posiada konkretnie
...
najstarszy element. Œredni¹ arytmetyczn¹ ostatnich wartoœci mo¿na obliczyæ dodaj¹c do wczeœniej obliczonej œredniej wszystkich przechowywanych elementów iloraz $\frac{New\, sample - Oldest\, sample}{Deep\, of\, queue}$  - jest to tzw. œrednia krocz¹ca. Wielkoœæ ta, jest zale¿na od szybkoœci nap³ywania danych z mikrokontrolera oraz 
zamierzonej iloœci ostatnich sekund, z których jest obliczana wspomniana œrednia.
\begin{lstlisting}
...
Jak opisano w podrozdziale 3.3 informacje o przeci¹¿eniach s¹ wysy³ane tylko w momencie ich wystêpowania, czyli wysokich wartoœci napiêcia zmierzonego na boczniku, natomiast 
...
periodycznie j¹ kopiuje i wstawia do kolejki, a tym samym obliczana œrednia spada zgodnie z za³o¿eniem, nie nadu¿ywaj¹c buforów mikrokontrolera.\\


\section{Inicjalizacja po³¹czenia i sterowanie serwami - ci¹g dalszy przyjêtego protoko³u danych}

W programie wykorzystano domyœlny modu³ do komunikacji szeregowej. Funkcje w nim zawarte pozwalaj¹ w ³atwy sposób otworzyæ port komunikacyjny, zainicjowaæ
...
\begin{enumerate}[{1.}]
	...
	\item Po zweryfikowaniu urz¹dzenia program wysy³a do mikrokontrolera swój kod (ten sam), a mikrokontroler porównuje has³o wbudowane i otrzymane. 
...
w "Visualowym ToolBoxie" jest Chart, czyli z jêzyka angielskiego "Wykres". Udostêpnia on  intuicyjny interfejs, w którym wystarczy wybraæ typ wykresu,
 linii ³¹cz¹cej jego punkty (np. spline), kolorystykê, legendê, a w trakcie dzia³ania programu dodawane s¹ wartoœci do aktualnego czasu i wpisywane w wykres. Dodatkowo za pomoc¹ 
wbudowanej w klasê funkcji wykres jest przeskalowywany wzglêdem osi Y dla lepszej czytelnoœci w przypadku d³u¿szego pojawiania siê podobnych wartoœci. 

\begin{figure}[H]
	\centering
	\includegraphics[width=12cm]{../Grafiki/ProgramCharts.jpg}
	\caption{Zrzut interfejsu programu. Na osi X oznaczane s¹ ostatnie próbki; na osi Y oznaczona jest wartoœæ przeci¹¿enia w jednostkach ADC.}
\end{figure}

\indent Interwa³ czasowy z jakim wprowadzane s¹ dane do wykresu musi byæ równy interwa³owi z jakim przychodz¹ dane z mikrokontrolera, 
a w³aœciwie z jakim dane wp³ywaj¹ do kolejki opisanej w podrozdziale 5.2 tak, aby wprowadzane wartoœci nie dublowa³y siê oraz nie zosta³y pominiête.\\
\begin{lstlisting}
...
Do aplikacji zaimplementowano dodatkow¹ funkcjonalnoœæ jak¹ jest mo¿liwoœæ sterowania ramieniem robota za pomoc¹ kontrolera do gier.
W tym celu wykorzystano otwarto Ÿród³owe API (Application Programming Interface) do DirectX o nazwie SlimDX, które jest przeznaczone g³ównie do tworzenia gier 
pod platform¹ .NET, ale w tym przypadku zosta³o u¿yte do oprogramowania kontrolerów. Interfejs, który udostêpnia SlimDX, jest uniwersalny i implementacja
...
\begin{figure}[H]
	...
	\caption{Zrzut interfejsu programu z wyszczególnieniem opcji wyboru urz¹dzenia steruj¹cego.}
\end{figure}
...
Prototypy linii monta¿owych w fabrykach, a w³aœciwie ramion przy nich pracuj¹cych to równie¿ przyk³ad, w którym wymagane jest kontrolowanie znacz¹cej liczby serw
...
Autor, pragnie z³o¿yæ ogromne wyrazy szacunku i podziêkowania swojego promotora
Pana doktora Grzegorza Zuzla, który sprawowa³ pieczê nad poczynaniami w pracy licencjackiej. Twór ten nie by³by te¿ mo¿liwy bez pozosta³ych pracowników
Zak³adu Doœwiadczalnej Fizyki Komputerowej z wyszczególnieniem Pana doktora Krzysztofa Pelczara. Ca³y ten sk³ad zaanga¿owa³ siê w rozwi¹zywanie napotkanych
problemów i to w³aœnie dziêki nim prezentowana praca zosta³a ukoñczona. Podziêkowania nale¿¹ siê równie¿ Gara¿owi Z³o¿onoœci, który pomimo licznych 
opracowywanych projektów, udostêpni³ opisywany w pracy mikrokontroler nale¿¹cy do gara¿owego asortymentu. 

\indent Przez okres ostatnich trzech lat autorowi by³o dane posi¹œæ ogromn¹ wiedzê, dlatego dziêkuje on wszystkim pracownikom wydzia³u, a zw³aszcza tym,
którzy potrafili rozbudziæ w nim pasjê do rzeczy, o których wczeœniej nie mia³ pojêcia, takie które pos³u¿¹ mu w przysz³oœci i wyró¿ni¹ go na rynku pracy. 
Nie zabrn¹³by on do miejsca, w którym aktualnie siê znajduje, gdyby nie dobrobyt i odpowiednie warunki do nauki, które zapewnili mu rodzice, dlatego z ca³ego
serca dziêkuje równie¿ im i obiecuje, ¿e zawsze bêdzie pracowa³ tak, aby byli z niego dumni. 
Autor jeszcze raz wszystkim ogromnie DZIÊKUJE.
...
\end{center}
 Noœnik danych do³¹czony do pracy zawiera:
\begin{itemize}
	\item Program napisany dla mikrokontrolera Arduino
	\item Program kontroluj¹cy napisany w jêzyku C\# 
